...
@PersistenceCapable( detachable = "true", table = "infinidat_device_filesystems_snapshots_controller_report")
public class INFINIDATDeviceFileSystemsSnapshotsReportImpl implements TabularReportGeneratorIf {
	private static Logger logger = Logger.getLogger( INFINIDATDeviceFileSystemsSnapshotsReportImpl.class );
	...
	public TabularReport getTabularReportReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		...
		for (int i = 0; i < objs.size(); i++) {
			...
			logger.info( "----#### INFINIDATDeviceVolumeReportImpl:getTabularReportReport ####---- ID: " + i  + " | TYPE: " + pojo.getDatasetType());
			if( pojo.getType().equals("SNAP")) {
