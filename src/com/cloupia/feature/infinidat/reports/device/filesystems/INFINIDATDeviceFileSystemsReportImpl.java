...
@PersistenceCapable( detachable = "true", table = "infinidat_device_filesystems_controller_report")
public class INFINIDATDeviceFileSystemsReportImpl implements TabularReportGeneratorIf {
	private static Logger logger = Logger.getLogger( INFINIDATDeviceFileSystemsReportImpl.class );

	@Override
	public TabularReport getTabularReportReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		if(INFINIDATConstants.DEBUG_FILESYSTEM && INFINIDATConstants.DEBUG){logger.info( "----#### INFINIDATDeviceVolumeReportImpl:getTabularReportReport ####----" );}
		...
		for (int i = 0; i < objs.size(); i++) {
			...
			if(INFINIDATConstants.DEBUG_FILESYSTEM && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATDeviceVolumeReportImpl:getTabularReportReport ####---- ID: " + i  + " | TYPE: " + pojo.getDatasetType());}
			if( pojo.getType().equals("MASTER")) {
