...
public class INFINIDATAddNewFileSystemForm {
	@FormField(label = "Name", help = "Enter the name for the new FileSystem", mandatory = true)
	@UserInputField(type = WorkflowInputFieldTypeDeclaration.GENERIC_TEXT)
	private String name;
	@FormField(label = "Pool", help = "Select the pool for the new FileSystem", type = FormFieldDefinition.FIELD_TYPE_EMBEDDED_LOV, lovProvider = INFINIDATPoolProvisionTypeProvider.NAME)
	private int poolID;
	...
	@FormField(label = "Size GB", help = "Enter the size of the new FileSystem", type = FormFieldDefinition.FIELD_TYPE_NUMBER, minValue = 1, maxValue = INFINIDATConstants.INFINIDAT_MAX_VOLUME_SIZE, mandatory = true)
	private long size;
	@FormField(label = "SSD Cache", help = "Enable SSD Cache for new FileSystem", type = FormFieldDefinition.FIELD_TYPE_BOOLEAN)
