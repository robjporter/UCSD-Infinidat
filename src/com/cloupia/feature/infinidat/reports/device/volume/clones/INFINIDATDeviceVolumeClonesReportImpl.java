...
public class INFINIDATDeviceVolumeClonesReportImpl implements TabularReportGeneratorIf {
	private static Logger logger = Logger.getLogger( INFINIDATDeviceVolumeClonesReportImpl.class );

	@Override
	public TabularReport getTabularReportReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		logger.info( "----#### INFINIDATDeviceVolumeClonesReportImpl:getTabularReportReport ####----" );
		...
		for (int i = 0; i < objs.size(); i++) {
			...
			logger.info( "----#### INFINIDATDeviceVolumeClonesReportImpl:getTabularReportReport ####---- ID: " + i  + " | TYPE: " + pojo.getVolumeType());
			if( pojo.getVolumeType().equals("CLONE")) {
