...
public class INFINIDATPieReportDiskPhysicalUsageImpl implements SnapshotReportGeneratorIf {
	...
	public SnapshotReport getSnapshotReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG){logger.info("----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- CONTEXT ID: " + context.getId()+context);}
		...
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- ACCOUNT NAME:" + accountName );}
		...
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- SIZE: " + objs.size() );}
		
		if( objs.size() == 1 ) {
			INFINIDATData pojo = objs.get( 0 );
			if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG2){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- DATA: " + pojo.debug() );}
			used_count = pojo.getUsedPhysicalCapacity();
			free_count = pojo.getFreePhysicalCapacity();
		}

		String data[] = myFormat.makeSmallerNumberRelativeToLargerWithMetric(used_count-free_count, free_count);
        
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- DATA 0: " + data[0] );}
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- DATA 1: " + data[1] );}
		if(INFINIDATConstants.DEBUG_GRAPHS && INFINIDATConstants.DEBUG1){logger.info( "----#### INFINIDATPieReportDiskPhysicalUsageImpl:getSnapshotReport ####---- DATA 2: " + data[2] );}
        ...
        cat.setCategoryName(INFINIDATConstants.REPORT_DISK_PHYSICAL_USAGE_LABEL);
