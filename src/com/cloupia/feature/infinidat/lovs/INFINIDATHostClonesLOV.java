...
public class INFINIDATHostClonesLOV  implements TabularReportGeneratorIf {
	private static Logger logger = Logger.getLogger( INFINIDATHostClonesLOV.class );
	public static final String HOSTS_TABULAR_PROVIDER = "infinidat_host_clone_tabular_provider";
	...
	public TabularReport getTabularReportReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		...
		if(elements.length == 2) {
			accName = elements[0];
			id = elements[1];
			logger.info( "----#### INFINIDATHostClonesLOV:getTabularReportReport ####---- ID: " + id );
			logger.info( "----#### INFINIDATHostClonesLOV:getTabularReportReport ####---- ACCNAME: " + accName );
		}
		...
		logger.info( "----#### INFINIDATHostClonesLOV:getTabularReportReport ####---- COUNT: " + count );
		...
		for( int i = 0; i < count; i++ ) {
			...
			if(myJson.getExactSingleItemAsString(response, "result", i + ".mapped").equals("false")) {
				if(myJson.getExactSingleItemAsString(response, "result", i + ".type" ).equals("CLONE")) {
