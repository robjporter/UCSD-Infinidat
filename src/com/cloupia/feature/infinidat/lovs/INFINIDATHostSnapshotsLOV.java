...
public class INFINIDATHostSnapshotsLOV  implements TabularReportGeneratorIf {
	private static Logger logger = Logger.getLogger( INFINIDATHostSnapshotsLOV.class );
	public static final String HOSTS_TABULAR_PROVIDER = "infinidat_host_snapshots_tabular_provider";
	...
	public TabularReport getTabularReportReport( ReportRegistryEntry reportEntry, ReportContext context ) throws Exception {
		...
		if(elements.length == 2) {
			accName = elements[0];
			id = elements[1];
			logger.info( "----#### INFINIDATHostSnapshotsLOV:getTabularReportReport ####---- ID: " + id );
			logger.info( "----#### INFINIDATHostSnapshotsLOV:getTabularReportReport ####---- ACCNAME: " + accName );
		}
		...
		logger.info( "----#### INFINIDATHostSnapshotsLOV:getTabularReportReport ####---- COUNT: " + count );
		...
		for( int i = 0; i < count; i++ ) {
			...
			if(myJson.getExactSingleItemAsString(response, "result", i + ".mapped").equals("false")) {
				if(myJson.getExactSingleItemAsString(response, "result", i + ".type" ).equals("SNAP")) {
